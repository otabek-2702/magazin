...
const onSubmit = () => {
  refForm.value?.validate().then(async ({ valid }) => {
    if (valid) {
      ...
      try {
        await axios.put(`/cashboxes/${props.id}`, {
          name: name.value,
...
const fetchDataById = async () => {
  ...
  try {
    const response = await axios.get(`/cashboxes/${props.id}`);

    if (response.status === 200) {
      name.value = response.data.cashbox.name;
      description.value = response.data.cashbox.description;
    }
  } catch (error) {
